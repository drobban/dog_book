defmodule DogBookWeb.RecordLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:registry_uid]} type="text" label="Registry uid" />
  ...
  def update(%{record: record} = assigns, socket) do
    changeset = Data.change_record(record)
  ...
  def handle_event("validate", %{"record" => record_params}, socket) do
    changeset =
      ...
      |> Data.change_record(record_params)
  ...
  defp save_record(socket, :edit, record_params) do
    case Data.update_record(socket.assigns.record, record_params) do
      {:ok, record} ->
  ...
  defp save_record(socket, :new, record_params) do
    case Data.create_record(record_params) do
      {:ok, record} ->
        notify_parent({:saved, record})
